...
const upload = () => {
  ...
  const handleAnalyze = async ({
    ...
    try {
      ...
      const response = await fetch("http://127.0.0.1:5000/api/resumes/analyze", {
        method: "POST",
        headers: {
          ...
          {isProcessing ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 animate-in fade-in duration-700">
              ...
              <p className="text-slate-500 text-center max-w-sm font-medium">
                Our AI model is cross-referencing your experience against industry standards and the job description...
              </p>
            </div>
          ) : (
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 w-full">
              <div className="mb-10 text-center">
                <h2 className="text-3xl md:text-4xl font-extrabold text-[#334155] tracking-tight mb-3">Analyze Your Resume</h2>
                <p className="text-slate-500 text-lg font-medium">Target a specific role to get highly tailored feedback.</p>
              </div>

              <form id="upload-form" onSubmit={handleSubmit} className="flex flex-col w-full gap-7">
                
                {/* 100% Width Input */}
                <div className="w-full flex flex-col gap-2">
                  <label htmlFor="company-name" className="text-xs font-extrabold text-[#0f766e] uppercase tracking-wider ml-1">
                    Company Name <span className="text-slate-400 font-medium normal-case tracking-normal">(Optional)</span>
                  </label>
                  <input
                    type="text"
                    name="company-name"
                ...
                {/* 100% Width Input */}
                <div className="w-full flex flex-col gap-2">
                  <label htmlFor="job-title" className="text-xs font-extrabold text-[#0f766e] uppercase tracking-wider ml-1">
                    Job Title
                  </label>
                  <input
                    type="text"
                ...
                {/* 100% Width Textarea */}
                <div className="w-full flex flex-col gap-2">
                  <label htmlFor="job-description" className="text-xs font-extrabold text-[#0f766e] uppercase tracking-wider ml-1">
                    Job Description <span className="text-slate-400 font-medium normal-case tracking-normal">(Optional)</span>
                  </label>
                  <textarea
                    rows={4}
                ...
                <div className="w-full flex flex-col gap-2 mt-2">
                  <label htmlFor="uploader" className="text-xs font-extrabold text-[#0f766e] uppercase tracking-wider ml-1">
                    Your Document
                  </label>
                  <div className={`w-full transition-all duration-300 rounded-2xl border-2 border-dashed ${file ? 'border-[#0f766e] bg-teal-50/50' : 'border-slate-300 bg-slate-50/50 hover:bg-slate-100/50 hover:border-slate-400'} p-2`}>
